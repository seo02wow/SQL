...
SELECT EMPLOYEE_ID,FIRST_NAME || ' ' || LAST_NAME AS NAME,
       SALARY,ROUND(SALARY * 1.123) AS INCREASE_SALARY,
...
SELECT FIRST_NAME || ' ' || LAST_NAME AS NAME,SALARY,
       ...
       ELSE 'Salary only' END AS TOTAL_SALARY, 
...
*/    
SELECT TO_CHAR(SUM(SALARY),'$999,999') AS SUM_SALARY,
...
*/
SELECT E.JOB_ID,AVG_SALARY
FROM EMPLOYEES E, (SELECT JOB_ID,AVG(SALARY) AVG_SALARY 
                    FROM EMPLOYEES 
                    WHERE JOB_ID NOT LIKE '%CLERK%'
                    GROUP BY JOB_ID) A
WHERE E.JOB_ID = A.JOB_ID
AND AVG_SALARY > 10000
ORDER BY AVG_SALARY DESC
;

SELECT JOB_ID,COUNT(*),AVG(SALARY)AVG_SALARY
FROM EMPLOYEES
GROUP BY JOB_ID
...
WHERE D.LOCATION_ID = L.LOCATION_ID --조인조건
  AND E.DEPARTMENT_ID = D.DEPARTMENT_ID -- 조인조건
  AND L.CITY = 'Oxford' -- 찾을 조건
;
-- 뷰(VIEW) 사용 
SELECT 'HR-Company' AS COMPANY_NAME,
       FIRST_NAME || ' ' || LAST_NAME AS NAME,
...
*/
SELECT D.DEPARTMENT_NAME,E.EMPLOYEES_CNT 
FROM DEPARTMENTS D, 
...
SELECT FIRST_NAME || ' ' || LAST_NAME AS NAME,D.DEPARTMENT_NAME,
       E.HIRE_DATE,E.SALARY,j.grade_level,J.LOWEST_SAL,J.HIGHEST_SAL
