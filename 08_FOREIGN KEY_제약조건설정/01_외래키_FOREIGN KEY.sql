...
------------------------------------
--(실습) EMP02, EMP03 테이블 외래키(FOREIGN KEY) 이름 변경 처리 
-- EMP02 : SYS_C007028 -> EMP02_EMPNO_FK 
ALTER TABLE EMP02 DROP CONSTRAINT SYS_C007028;
ALTER TABLE EMP02 ADD CONSTRAINT EMP02_EMPNO_FK 
            FOREIGN KEY (DEPTNO) REFERENCES DEPT (ID);
-- EMP 03 : EMP03_DEPTNO_FK -> FK_EMP03_DEPNO
ALTER TABLE EMP03 DROP CONSTRAINT EMP03_DEPTNO_FK;
ALTER TABLE EMP03 ADD CONSTRAINT FK_EMP03_DEPNO 
            FOREIGN KEY (DEPTNO) REFERENCES DEPT (ID);

--EMP03 테이블 
alter table "MADANG"."EMP03" drop constraint "FK_EMP03_DEPNO";
ALTER TABLE EMP03 ADD CONSTRAINT FK_EMP03_DEPNO 
            FOREIGN KEY (DEPTNO) REFERENCES DEPT (ID);
...
SELECT * FROM EMP02;
-- 제약조건 활성화(적용)
-- 제약조건 활성화 (또는 추가 시) 제약조건 위반되는 데이터가 있으면 활성화(추가) 안됨
...
INSERT INTO HSCHOOL VALUES(1,1,1,'을지문덕');  -- 중복 안 됨